...
import json, urllib.request, time, socket, ssl, urllib.parse, http.client , os.path
...
def main(argv):
	...
	#check to see if user supplied a second argument, if so use that as the output file
	if len(argv)==3:
		...
		for url in urllist:
			...
			if url[0:4] != 'http':
				...
				try:
					...
					print('\033[93m',"Warning: Malformed url on line ", line_number,'\033[0m',"\n")
					pass
			
			#Attempt to connect to the request URL
			try:
				#Set the start time to the current number of seconds since epoch
